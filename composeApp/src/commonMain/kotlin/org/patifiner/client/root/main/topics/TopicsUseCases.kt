...
import org.patifiner.client.root.main.topics.data.AddUserTopicRequest
import org.patifiner.client.root.main.topics.data.AddUserTopicsRequest
...
import org.patifiner.client.root.main.topics.add.ui.mapTreeToViewModel
        ...
        withContext(Dispatchers.Default) { // mapping could be heavy
            coroutineScope {
                val topicsDeferred = async {
                    repo.loadTopicsTree().getOrThrow()
                }
                val userTopicsDeferred = async {
                    repo.getUserTopics().getOrThrow()
                }
...
class SearchTopicsUseCase(private val repo: TopicsRepository) {
    suspend operator fun invoke(
        query: String,
        userTopicsTree: List<TopicViewModel>
    ): Result<List<TopicViewModel>> = runCatching {
        fun flatten(node: TopicViewModel): List<TopicViewModel> =
            listOf(node) + node.children.flatMap(::flatten)

        val fullTopicMap: Map<Long, TopicViewModel> = userTopicsTree.flatMap(::flatten)
            .associateBy { it.id }
...
class AddUserTopicUseCase(private val repo: TopicsRepository) {
    suspend operator fun invoke(
        ...
        draft: UserTopicInfo
    ): Result<List<UserTopicDto>> = runCatching {
...
class RemoveUserTopicUseCase(private val repo: TopicsRepository) {
    suspend operator fun invoke(req: RemoveUserTopicsRequest): Result<Int> =
        repo.removeUserTopics(req)
}
